...
let noactE = ["/explore","/settings","/messages","/notifications","/timeline","/lists","/verified-choose","/communities"]
let noactI = ["/explore/","/notifications/","/compose/post","/status/","/i/premium_sign_up"]
let actflag;
...
//戻り時のスクロール位置制御（気休め）
const mainTL= document.querySelectorAll('div[aria-label="タイムライン: ホームタイムライン"]')
let oldscroll;
let scrollhide = function(){
oldscroll = mainTL.scrollTop;
    oldscroll = window.scrollY
}
let scrollshow = function(){
    if(oldscroll =!null){
    mainTL.scrollTop == oldscroll;}
}
addEventListener('pgehide',scrollhide)
addEventListener('pageshow',scrollshow)
